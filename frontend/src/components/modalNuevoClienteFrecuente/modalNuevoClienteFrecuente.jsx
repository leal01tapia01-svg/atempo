...
const ModalNuevoClienteFrecuente = ({ modo = 'crear', cliente, onClose }) => {
  ...
  const buscarSugerencias = (q) => {
    ...
    setLoading(true);
    fetch(`/api/citas/sugerencias?nombre=${encodeURIComponent(q)}`, {
      method: 'GET',
  ...
  const handlePrimary = async () => {
    ...
    try {
      ...
      if (modo === 'editar' && form.id) {
        res = await fetch(`/api/clientes-frecuentes/${form.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify(payload)
        });
      } else {
        res = await fetch('/api/clientes-frecuentes', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify(payload)
        });
  ...
  const handleEliminar = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await fetch(`/api/clientes-frecuentes/${form.id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
