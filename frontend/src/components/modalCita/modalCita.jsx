...
const ModalCita = ({ modo = 'crear', cita = {}, onClose, onGuardar, onEliminar }) => {
    ...
    const cargarDatos = async () => {
      try {
        ...
        const [resEmp, resClientes] = await Promise.all([
          ...
          fetch('https://biological-ariel-atempo-05d801c3.koyeb.app/api/clientes-frecuentes', { headers: { Authorization: `Bearer ${token}` } }) 
        ]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (modo === 'editar' && cita?.id) {
        ...
        res = await fetch(`https://biological-ariel-atempo-05d801c3.koyeb.app/api/citas/${cita.id}`, {
          method: 'PUT',
          headers: {
        ...
        res = await fetch('https://biological-ariel-atempo-05d801c3.koyeb.app/api/citas', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`
  ...
  const confirmarEliminar = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`https://biological-ariel-atempo-05d801c3.koyeb.app/api/citas/${cita.id}`, {
        method: 'DELETE',
